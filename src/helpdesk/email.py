...
def process_queue(q, logger):
    logger.info(
        f"***** {ctime()}: Begin processing mail for django-helpdesk queue: {q.title}"
    )
...
def send_info_email(
    message_id: str, f: FollowUp, ticket: Ticket, context: dict, queue: dict, new: bool
):
...
def mime_content_to_string(
    ...
    if part["Content-Transfer-Encoding"] in [None, "8bit", "7bit"] and (
        charset == "utf-8" or charset is None
    ):
...
def extract_email_message_content(
    ...
    if "multipart/related" == content_type:
        ...
        message_part = message_part.get_body(
            preferencelist=[
                "html",
                "plain",
            ]
    ...
    if "text/html" == content_type:
        ...
        plain_message_part = parent_part.get_body(
            preferencelist=[
                "plain",
            ]
        )
...
def extract_email_subject(
    ...
    subject = decode_mail_headers(decodeUnknown(email_msg.get_charset(), subject))
...
def extract_attachments(
    ...
    if (
        "multipart" == content_maintype
        and target_part.get_content_disposition() not in ["inline", "attachment"]
    ):
        ...
        # The iterator must be different depending on whether we have already excluded message content parts
            ...
            for part in target_part.iter_parts():
                if not content_parts_excluded and part.get_content_type() in [
                    "text/plain",
                    "text/html",
                ]:
